...
export function useBatchAutomation(batchId?: string) {
  ...
  const runBatch = async (batch: any, platform: string, options?: { delay?: number; maxRetries?: number }) => {
    if (!batch || !batch.id) throw new AutoPromptrError('No batch provided', 'NO_BATCH_PROVIDED');
  ...
  const stopBatch = async () => {
    if (!batchId) throw new AutoPromptrError('No batch ID provided', 'NO_BATCH_ID');
