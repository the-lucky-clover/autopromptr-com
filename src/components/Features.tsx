        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {features.map((feature, index) => {
            ...
            return (
              <Card 
                key={index} 
                className={`glass-effect border-purple-500/20 hover:border-purple-500/40 transition-all duration-300 hover-scale animate-slide-up ${feature.delay} rounded-2xl bg-black/20 backdrop-blur-sm`}
              >
                <CardHeader className="rounded-t-2xl pb-4">
                  <div className="w-12 h-12 bg-gradient-to-r from-purple-500 to-blue-500 rounded-2xl flex items-center justify-center mb-4 flex-shrink-0">
                    <IconComponent className="w-6 h-6 text-white" />
                  </div>
                  <CardTitle className="text-lg sm:text-xl text-white leading-tight">{feature.title}</CardTitle>
                </CardHeader>
